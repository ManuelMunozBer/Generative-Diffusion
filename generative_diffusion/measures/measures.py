...
def calculate_bpd(
    ...
    *,
    ...
    """
    Aproximación de Bits per Dimension (bpd) mediante denoising score matching.

    Parámetros:
        score_model (nn.Module): Modelo que predice el score ∇log p(x_t | t)
        sde (objeto): Proceso SDE con método marginal_prob(x, t)
        images (Tensor): Tensor de imágenes generadas en rango [-1, 1],
        con forma (N, C, H, W)
        batch_size (int): Tamaño del batch
        n_samples (int): Número máximo de muestras a evaluar
        device (str): Dispositivo para evaluación

    Retorna:
        float: Bits per dimension (menor ⇒ mejor)
    """
    score_model.eval()
    ...
    for i in tqdm(range(0, n, batch_size), desc="BPD"):
