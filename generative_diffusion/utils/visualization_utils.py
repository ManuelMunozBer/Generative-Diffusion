...
# --------------------------------------------------------------------- #
# Graficar evolución de ruido a imagen                                  #
# --------------------------------------------------------------------- #


def plot_image_evolution(
    images: torch.Tensor,  # shape: [steps, batch_size, 3, H, W]
    n_images: int,
    ...
    """
    Visualiza la evolución de imágenes RGB a lo largo de pasos intermedios.

    Parámetros:
        images: Tensor con shape [steps, batch_size, 3, H, W]
        n_images: número de imágenes a mostrar (debe ser <= batch_size)
        n_intermediate_steps: lista de pasos (indices) a visualizar
    ...
    for n_image in range(n_images):
        for j, step in enumerate(n_intermediate_steps):
            # Extrae imagen RGB en ese paso: [3, H, W] -> [H, W, 3]
            img = images[step, n_image]  # [3, H, W]
            img = img.permute(1, 2, 0).cpu().numpy()  # [H, W, 3]
            axs[n_image, j].imshow(img)
            axs[n_image, j].set_axis_off()

            # Añadir título a la izquierda de la fila
            if titles is not None and n_image < len(titles):
                axs[n_image, 0].set_title(
                    titles[n_image], loc="left", fontsize=12, pad=10
                )
